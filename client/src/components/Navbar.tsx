...
export default function Navbar() {
  const { t } = useI18n()
  const [open, setOpen] = useState(false) // 모바일 전용 오프캔버스 상태
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            {/* 오프캔버스 패널 (화면의 약 1/3 차지, 너무 좁지 않게 최소폭 지정) */}
            <motion.aside
              key="mobile-sidebar"
      ...
      {/* ⏺ 데스크탑 전용: 기존 고정 사이드바 (md 이상에서만 보임) */}
      <motion.aside
