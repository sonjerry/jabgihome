// client/src/components/ModelViewer.tsx
import {
  FC,
  Suspense,
  useRef,
...
const GltfContent: FC<{ url: string }> = ({ url }) => {
  const { scene } = useGLTF(url)
  return <primitive object={scene.clone()} />
}
const FbxContent: FC<{ url: string }> = ({ url }) => {
  const obj = useFBX(url)
  return <primitive object={obj.clone()} />
}
const ObjContent: FC<{ url: string }> = ({ url }) => {
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  }, [camera, fadeIn, initPitch, initYaw, pivot, onLoaded, url, autoFrame])

  // ⬇⬇ 핵심 수정: isTouch 체크 제거 → 데스크톱에서 드래그 회전 정상 동작
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const down = (e: PointerEvent) => {
      ...
      if (pts.size === 1) {
        ...
        startZ = (camera as THREE.PerspectiveCamera).position.z
        e.preventDefault()
      }
      invalidate()
    }
    ...
    const move = (e: PointerEvent) => {
      ...
      if (mode === "rotate") {
        ...
        cam.position.z = THREE.MathUtils.clamp(startZ * ratio, minZoom, maxZoom)
        invalidate()
      }
    }
  ...
  const capture = () => {
    ...
    s.traverse((o: any) => {
      if (o.isLight && "castShadow" in o) {
        tmp.push({ l: o, cast: (o as any).castShadow })
    ...
    tmp.forEach(({ l, cast }) => ((l as any).castShadow = cast))
